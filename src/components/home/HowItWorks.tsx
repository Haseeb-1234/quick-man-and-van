    ...
    <section className="border-t border-[var(--border)] bg-surface py-16 sm:py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="max-w-2xl">
          <h2 className="font-display text-3xl font-bold tracking-tight text-primary">How it works</h2>
          <p className="mt-3 text-lg text-secondary">Three simple steps from quote to confirmed booking.</p>
        </div>
        <ol className="mt-12 grid gap-8 sm:grid-cols-3">
          {steps.map(({ step, title, body }) => (
            <li key={step} className="surface-card relative overflow-hidden rounded-2xl p-6">
              <span className="absolute right-5 top-2 font-display text-5xl font-bold text-accent/[15%]" aria-hidden>
                {step}
              </span>
              <span className="relative flex size-10 items-center justify-center rounded-full bg-accent/[12%] text-sm font-bold text-accent">
                {step}
              </span>
              <h3 className="relative mt-4 text-lg font-semibold text-primary">{title}</h3>
